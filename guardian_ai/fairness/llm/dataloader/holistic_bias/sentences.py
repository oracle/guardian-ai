...
from .util import NO_PREFERENCE_DATA_STRING, NONE_STRING, RANDOM_SEED
...
class HolisticBiasSentenceGenerator:
    ...
    def get_dataset_folder(self, dataset_version: str) -> str:
        """
        Get the path to the folder containing the dataset, given the input version
        string.
        """
        dataset_folder = os.path.join(self.save_folder, dataset_version)
        return dataset_folder

    def get_descriptors(self, dataset_version: str) -> Dict[str, dict]:
        """
        Get all descriptors, given the input version string.
        """
        dataset_folder = self.get_dataset_folder(dataset_version)
        descriptors_path = os.path.join(dataset_folder, "descriptors.json")
        with open(descriptors_path) as f:
            descriptors = json.load(f)
        return descriptors

    def get_nouns(self, dataset_version: str) -> Dict[str, list]:
        """
    ...
    def get_sentence_templates(self, dataset_version: str) -> Dict[str, dict]:
        """
    ...
    def get_standalone_noun_phrases(self, dataset_version: str) -> Dict[str, list]:
        """
    ...
    def get_compiled_noun_phrases(self, dataset_version: str) -> pd.DataFrame:
        """
        ...
        no_descriptor_template = self.WITH_NOUN_TEMPLATE.replace(" {descriptor}", "")
        # For instance, this will allow for "I'm a man" as a control for "I'm a blind
        # man"
        no_descriptor_noun_phrase_metadata = []
        for group_gender, gender_noun_tuples in self.get_nouns(dataset_version).items():
            for noun, plural_noun in gender_noun_tuples:
                ...
                no_descriptor_noun_phrase_metadata.append(
                    {
                        "axis": "null",
                        "bucket": self.NONE_STRING,
                        "descriptor": self.NONE_STRING,
                        "descriptor_gender": self.NONE_STRING,
                        "descriptor_preference": self.NONE_STRING,
                        "noun": noun,
        ...
        for axis, axis_descriptors in self.get_descriptors(dataset_version).items():
            ...
            for bucket, descriptor_info in axis_descriptors.items():
                for descriptor_obj in descriptor_info:
                    this_axis_noun_phrase_metadata += self._get_noun_phrase_metadata(
        ...
        standalone_noun_phrases = self.get_standalone_noun_phrases(dataset_version)
        for axis, axis_noun_phrases in standalone_noun_phrases.items():
            for noun_phrase_obj in axis_noun_phrases:
                ...
                noun_phrase_preference = noun_phrase_obj.get(
                    "preference", self.NO_PREFERENCE_DATA_STRING
                )
                ...
                if "{noun}" in possibly_templated_noun_phrase:
                    nouns = self.get_nouns(dataset_version)
                    noun_phrase_metadata = [
                        {
                            ...
                            "noun_phrase": possibly_templated_noun_phrase.format(
                                article=self._get_article(noun), noun=noun
                            ),
                    ...
                    noun_phrase_metadata = [
                        {
                            ...
                            "descriptor_gender": self.NONE_STRING,
                            "descriptor_preference": noun_phrase_preference,
                            "noun": self.NONE_STRING,
                            "plural_noun": self.NONE_STRING,
                            "noun_gender": "neutral",
        ...
        noun_phrase_df = pd.DataFrame(all_noun_phrase_metadata)[
            self.SORT_COLUMNS
        ].sort_values(self.SORT_COLUMNS)

        return noun_phrase_df

    def _get_article(self, descriptor: str) -> str:
        """
    ...
    def _get_noun_phrase_metadata(
        ...
        descriptor_gender = descriptor_obj.get("gender", self.NONE_STRING)
        # Set the gender associated with the descriptor, if any
        descriptor_article = descriptor_obj.get("article", self._get_article(descriptor))
        ...
        all_noun_phrase_metadata = []
        for template in self.NOUN_PHRASE_TEMPLATES:
